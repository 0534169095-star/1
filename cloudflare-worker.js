...
async function requireUser(request, env, allowedRoles = null) {
  ...
  const isInitialSuperAdmin = INITIAL_SUPER_ADMIN_EMAIL_SHA256S.has(await sha256(account.email));
...
async function dataActor(request, env) {
  const idToken = getBearerToken(request);
  const account = await verifyGoogleAccount(idToken, env);
  const initialAdmin = INITIAL_SUPER_ADMIN_EMAIL_SHA256S.has(await sha256(account.email));
